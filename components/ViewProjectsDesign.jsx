  ...
  return (
    <section className="py-5 mx-auto relative">
      <div
        ...
        onMouseEnter={() => clearInterval(intervalRef.current)} // Desktop Hover
        onMouseLeave={() => {
          intervalRef.current = setInterval(() => {
            setCurrent((prev) => (prev + 1) % projects.length);
          }, 8000);
        }}
        onTouchStart={() => clearInterval(intervalRef.current)} // Mobile Touch
        onTouchEnd={() => {
          intervalRef.current = setInterval(() => {
            setCurrent((prev) => (prev + 1) % projects.length);
          }, 8000);
          ...
          >
            <div className="relative w-full h-full bg-cover bg-center rounded-xl overflow-hidden" style={{ backgroundImage: `url(${projects[current].image})` }}>
              <div className="bg-white absolute bottom-0 m-4 p-4 rounded-xl opacity-90">
                <h3 className="text-md xl:text-xl font-semibold">{projects[current].title}</h3>
                <p className="text-xs xl:text-sm mt-2">{projects[current].description}</p>
